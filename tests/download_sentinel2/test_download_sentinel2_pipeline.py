...
def _sentinel2_params(**overrides):
    params = {
        ...
        "drive_export": {
            ...
            "file_name_prefix": "sentinel2_test",
...
def test_download_sentinel2_pipeline_has_expected_node_count():
    assert len(create_pipeline().nodes) == 7


def test_google_earth_engine_config_is_validated_and_grouped():
    config = validate_google_earth_engine_config(
...
def test_google_earth_engine_config_accepts_service_account():
    config = validate_google_earth_engine_config(
        params_gee=_gee_params(
...
def test_google_earth_engine_config_rejects_invalid_values(
    gee_overrides, download_overrides, match
):
