...
# Funcion para guardar la matriz de confusion multiclase como PNG.
def plot_mining_multiclass_confusion_matrix(
    mining_multiclass_metrics: dict[str, Any],
    mining_multiclass_random_forest_config: dict[str, Any],
) -> dict[str, Any]:
    plot_params = mining_multiclass_random_forest_config.get("confusion_matrix_plot", {})
    output_path = Path(plot_params.get("output_path", "data/08_reporting/train_mining_multiclass_rf/mining_multiclass_confusion_matrix.png"))
    output_path.parent.mkdir(parents=True, exist_ok=True)
    matrix = np.array(mining_multiclass_metrics["confusion_matrix"])
    labels = mining_multiclass_metrics["labels"]
    ...
    axis.figure.colorbar(image, ax=axis, fraction=0.036, pad=0.02)
    ...
    axis.set_title(plot_params.get("title", "Matriz de confusion multiclase"))
