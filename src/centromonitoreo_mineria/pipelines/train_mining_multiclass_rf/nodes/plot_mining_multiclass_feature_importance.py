...
# Funcion para guardar la importancia de variables multiclase como PNG.
def plot_mining_multiclass_feature_importance(
    mining_multiclass_feature_importance: pd.DataFrame,
    mining_multiclass_random_forest_config: dict[str, Any],
) -> dict[str, Any]:
    plot_params = mining_multiclass_random_forest_config.get("feature_importance_plot", {})
    output_path = Path(plot_params.get("output_path", "data/08_reporting/train_mining_multiclass_rf/mining_multiclass_feature_importance.png"))
    output_path.parent.mkdir(parents=True, exist_ok=True)
    top_n = int(plot_params.get("top_n", 20))
    data = mining_multiclass_feature_importance.head(top_n).sort_values("importance")
